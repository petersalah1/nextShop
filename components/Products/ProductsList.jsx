      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {allProducts.map((product) => (
            <Link key={product._id} href={`/products/${product._id}`} className="group overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              <div className="aspect-square overflow-hidden bg-gray-100">
                <img src={product.imageCover} alt={product.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-110" />
              </div>

              <div className="p-4">
                <p className="text-xs font-medium text-gray-400">{product.category?.name}</p>

                <h3 className="mt-2 line-clamp-2 min-h-10 text-sm font-semibold text-gray-950">{product.title}</h3>

                <div className="mt-3 flex items-center justify-between gap-3">
                  <p className="font-bold text-(--primary)">{product.price} EGP</p>

                  <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-600">⭐ {product.ratingsAverage}</span>
                </div>
              </div>
            </Link>
          ))}
